...
typedef void (EvoControllerLean::*Fptr) ();
long profile(Fptr);
...
#ifdef CPSEL
bool GPU_SELECT_POINTS = false;
#else
bool GPU_SELECT_POINTS = true;
#endif

#ifdef ENABLE_TEST
	bool TEST_TEAMS = true;
#else
	bool TEST_TEAMS = false;
#endif
EvoControllerLean::EvoControllerLean(map < string, string> &args)
{
   DE = new DataEngine(args);
   useGPU = true;
   initializeGpuEnv(DE->learnerPopulationSize,DE->pointPopulationSize, DE->maxProgSize,DE->pointDim);
   allocateDevicePointMatrix();
   allocateDeviceBidMatrix();
   setConstants(DE->pointPopulationSize,DE->learnerPopulationSize,DE->maxProgSize+1,DE->pointDim );
...
EvoControllerLean::~EvoControllerLean()
{
   destroyGpuEnv();
   delete (DE);
}
...
// Main GP Loop
/////////////////////////////////
long profile(Fptr f, EvoControllerLean* o, char* name)
{
    struct timeval tv_start;
    struct timeval tv_end;
    struct timezone tz;

    //char* name = const_cast<char*>(str);
	gettimeofday(&tv_start, &tz);
	startNVTXRange(name);
	(o->*f)();
    stopNVTXRange(name);
    gettimeofday(&tv_end, &tz);
    return Diff(tv_start,tv_end);
}


void EvoControllerLean::runTests()
{
	...
	if(useGPU == false)
	{
		...
		pushPointMatrixToDevice(DE->pointMatrix, DE->pointPopulationSize);

	        EvaluateLearners( DE->learnerMatrix,DE->learnerBidMatrix,0, DE->learnerPool->maxUsedIndex()+1,  DE->pointPool->size());
	        copyBidMatrixToHost(DE->learnerBidMatrix);
...
void EvoControllerLean::run()
{
    ...
    if(TEST_TEAMS)
    {
    	timerTest = profile(&EvoControllerLean::testTeams, this, "Test");
    }
    printf("TESTTIME %lu\n", timerTest);
...
void EvoControllerLean::initPoints()
{
   DE->initPoints();
}

void EvoControllerLean::initTeams()
{
   DE->initTeams(time);
}
...
void EvoControllerLean::evaluate()
{
    ...
    if (useGPU)
    {
 //       cout << "Enter GPU EVAL" << endl;
    	pushPointMatrixToDevice(DE->pointMatrix, DE->pointPopulationSize);
    	//_learnerBid* A = new _learnerBid[DE->learnerPopulationSize* DE->pointPopulationSize];
    	//for(int i=0;i<600;i++){A[i]=i;}

    	//pushBidMatrixToDevice(A,DE->learnerPopulationSize);
    	//copyBidMatrixToHost(DE->learnerBidMatrix);
        EvaluateLearners( DE->learnerMatrix,DE->learnerBidMatrix,0, DE->learnerPool->maxUsedIndex()+1,  DE->pointPool->size());
        copyBidMatrixToHost(DE->learnerBidMatrix);
        //DE->printBidMatrix();
    ...
    profile(&EvoControllerLean::EvaluateTeams, this,"Eval-Teams");
...
void EvoControllerLean::EvaluateTeams()
{
	int val = 0;
...
float EvoControllerLean::evalTeam(int team_index, int point_index)
{
...
void EvoControllerLean::getDist(map < _pointIndex, vector < short> * > *dist)
{
...
template < class vtype > bool EvoControllerLean::dominated(vector < vtype > &is, /* Is this vector */
...
template < class ptype > void EvoControllerLean::selParetoDebug(set < ptype  > &from,
...
template < class ptype > void EvoControllerLean::selPareto(set < ptype  > &from,
                         map < ptype , double > &score,
                         int nrem,
                         set < ptype > &toDel)
{
...
template <class keyType, class valType > void EvoControllerLean::findParetoFront(map<keyType,vector<valType> *> dist, set<keyType> &F, set<keyType>  &D, double epsilon)
{
...
template <class keyType, class valType > void EvoControllerLean::findParetoFrontDEBUG(map<keyType,vector<valType> *> *dist, set<keyType> &F, set<keyType>  &D, double epsilon,int testwhich)
{
    typename map<keyType, vector< valType>* > ::iterator veiter1, veiter2, veiterbegin, veiterend;
...
//    cout << endl;
 ...
 template <class ptype, class vtype > void EvoControllerLean::sharingScore(map < ptype , vector < vtype > * > &outMap,
                                    map < ptype , double > &score,
...
void EvoControllerLean::selectPoints()
{


	//  DE->printTeamRewardMatrix();
bool CPU_SELECT_POINTS = true;
	if (GPU_SELECT_POINTS)
	{
		thrust::host_vector<_teamReward> hToDel(DE->pointGenerationGapSize);
	...
	else if (CPU_SELECT_POINTS)
	{
		...
		int pointCount = DE->pointPopulationSize;
		int teamCount = DE->teamPopulationSize;


		bool* dist = new bool[pointCount*teamCount*teamCount];
		 gettimeofday(&tv_start, &tz);
		// GetDist
//			for(int pIndex=pointCount-1;pIndex>=0;--pIndex){
//				for(int i=teamCount-1;i>=0;--i){
//					for(int j=teamCount-1;j>=0;--j){
//						dist[pIndex*teamCount*teamCount +i*teamCount+j] = DE->teamRewardMatrix[i*pointCount+pIndex] > DE->teamRewardMatrix[j*pointCount+pIndex];
//					}
//				}
//			}
		...
		gettimeofday(&tv_end, &tz);
		timerDist = Diff(tv_start,tv_end);
		       	printf("PSTAT %ld\n",timerDist);

			set<int>::iterator it = DE->pointPool->begin();
			advance(it,DE->pointGenerationGapSize);
			pointsToDel.insert(DE->pointPool->begin(),it);
...
void EvoControllerLean::selectTeams()
{
	...
	if(GPU_SELECT_TEAMS)
	{
		thrust::host_vector<_teamReward> hToDel(DE->pointGenerationGapSize);
		thrust::fill(hToDel.begin(),hToDel.end(),-1);
		SelectTeamsGPU(DE->teamPopulationSize,DE->pointPopulationSize,DE->teamRewardMatrix,DE->teamGenerationGapSize,&hToDel);
//
//		// initialize indices vector to [0,1,2,..]
//		thrust::counting_iterator<int> iter(0);
//		thrust::device_vector<int> indices(numElement);
...
//        }
        ...
        nrem = D.size() - (keep - F.size());
        selParetoDebug(D, score, nrem, teamsToDel);
    }
    else
    {
    	printf(" F == TOO BIG\n");
...
//        }
        ...
        nrem = F.size() - keep;
      //  cout << "nrem: " << nrem << endl;
        selParetoDebug(F, score, nrem, teamsToDel);
...
void EvoControllerLean::cleanup()
{
...
void EvoControllerLean::testTeams_gpu()
{
...
double EvoControllerLean::testTeamAgainstTestingSet_gpu(_teamIndex tindex)
{
...
void EvoControllerLean::testTeams_cpu()
{
...
double EvoControllerLean::testTeamAgainstTestingSet_cpu(_teamIndex tindex)
{
...
double EvoControllerLean::testTeamAgainstTrainingSet(_teamIndex tindex)
{
...
void EvoControllerLean::testTeam(_teamIndex tindex)
{
...
void EvoControllerLean::testTeams()
{
//	useGPU = false;
//    if (useGPU)
//    {
...
void EvoControllerLean::stats(int t)
{
    int sumTeamSizes = 0;
    int nrefs = 0;
